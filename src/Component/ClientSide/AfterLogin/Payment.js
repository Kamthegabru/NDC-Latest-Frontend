...
const PaymentInformation = () => {
  ...
  useEffect(() => {
    if (userData?.paymentData) {
      ...
      setLoading(false);
    } else if (userData && !userData.paymentData) {
      // If userData exists but no payment data, stop loading
      setLoading(false);
    }
  }, [userData]);
  ...
  const handleChange = (e) => {
    ...
    if (name === "creditCardNumber") {
      ...
      // Only allow up to 9 digits for routing number
      const digitsOnly = value.replace(/\D/g, "").slice(0, 9);
      setTempPaymentInfo(prev => ({ ...prev, [name]: digitsOnly }));
    } else if (name !== "accountName" && name !== "accountType") {
      // Allow only digits for other numeric fields
      const digitsOnly = value.replace(/\D/g, "");
      setTempPaymentInfo(prev => ({ ...prev, [name]: digitsOnly }));
    } else {
      // For text fields like accountName and accountType
            ...
            {['creditCardNumber', 'cvv', 'expMonth', 'expYear', 'billingZip'].map((key) => (
              <Grid item xs={12} sm={key === 'creditCardNumber' ? 12 : 6} md={key === 'creditCardNumber' ? 8 : 4} key={key}>
                <TextField
                  ...
                  name={key}
                  value={key === 'creditCardNumber' ? formatCardNumber(tempPaymentInfo[key] || "") : (tempPaymentInfo[key] || "")}
                  ...
                  inputProps={{
                    ...
                    maxLength: key === 'creditCardNumber' ? 19 : key === 'cvv' ? 3 : key === 'expMonth' ? 2 : key === 'expYear' ? 4 : key === 'billingZip' ? 5 : undefined
                  }}
